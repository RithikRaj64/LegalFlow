...
st.title("⚖️ The Constitution Chatbot")
...
for message in st.session_state.central_messages:
...
# with button_placeholder.container():
    ...
    #         st.session_state.central_messages.append({"role": "user", "content": text})
    ...
    #         st.session_state.central_messages.append({"role": "assistant", "content": response})
...
if prompt := st.chat_input("Enter your query"):
    # Add user message to chat history
    st.session_state.central_messages.append({"role": "user", "content": prompt})
    # Display user message in chat message container
    with st.chat_message("user"):
        st.markdown(prompt)
